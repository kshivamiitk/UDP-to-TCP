...
int main(int argc, char **argv) {
    ...
    svr.sin_port = htons(static_cast<uint16_t>(port));
    ...
    while (true) {
        ...
        if (!handle_handshake(sock)) {
            log_message("SERVER", "Handshake failed - continue");
            continue;
        }
        ...
        while (!finished) {
            ...
            if (!pkt::parse_packet(rbuf, (size_t)r, flags, seq, ack, wnd, payload)) {
                log_message("SERVER", "Invalid or corrupted packet - ignoring");
                continue;
            }
            if (seq == expected) {
                if (!payload.empty()) {
                ...
                std::vector<uint8_t> ackpkt = pkt::build_packet(pkt::F_ACK, seq, 0, wnd, {});
                sendto(sock, reinterpret_cast<const char*>(ackpkt.data()), ackpkt.size(), 0,
