...
class Proxy:
    def __init__(self, fport, back_port, log_path, ready_file, seq_drop=3, idle=6.0):
        ...
        self.server=("127.0.0.1", back_port); self.client=None
        self.seq_drop=seq_drop; self.dropped=False; self.q=[]; self.qid=0
        ...
        while True:
            ...
            for s in r:
                ...
                if s is self.front:
                    ...
                    if h:
                        f,seq,_=h
                        if (f & F_DATA) and (seq==self.seq_drop) and not self.dropped:
                            self.dropped=True
                            continue
...
if __name__=="__main__":
    ...
    ap.add_argument("--drop-seq",type=int,default=3)
    a=ap.parse_args()
